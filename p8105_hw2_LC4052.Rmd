...
The resulting dataset contains information from two zillow datasets (`r zipcodes_df`and`r zori_df`) with `r length(combined_data)` observations and `r ncol(combined_data)` variables. Each observation has its identical variables `r county`,`r zip_code`,`r file_date`, and some other variables such as`r rent_price`,`r neighborhood`, and `r region_id`, etc.
